...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const m = dict.immobilienServices.meta;
  return {
    title: m.title,
...
export default async function ImmobilienServicesPage() {
  const locale = await getLocaleFromHeaders();
  const dict = await getDictionary(locale);
  const s = dict.immobilienServices;
  const prefix = `/${locale}`;
  ...
  return (
    <>
      ...
      <section id="leistungen" aria-label={s.modulesHeading}>
        <ServicesPackagesSection dict={s} lang={locale} />
      </section>
